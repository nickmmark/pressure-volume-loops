...
## Effects of Mechanical Circulatory Support (MCS) Devices
### IABP
An Intra-aortic Balloon Pump (IABP) improves cardiac function by shifting the left ventricular (LV) pressure-volume (PV) loop leftward and downward, indicating reduced myocardial workload and oxygen demand. This is achieved through diastolic augmentation (increasing coronary perfusion pressure) and systolic unloading (reducing afterload), resulting in decreased end-systolic pressure and volume (ESV): 
* Leftward/Downward Shift: The loop shifts to lower volumes and lower pressures, reflecting a smaller end-systolic volume (ESV) and reduced afterload.
* Reduced Myocardial Work: The area within the loop, which represents stroke work (SW), decreases, while overall myocardial oxygen demand is reduced.
* Increased Diastolic Pressure: Diastolic augmentation increases the coronary perfusion pressure (CPP).
* Decreased Afterload: The pressure required for the LV to open the aortic valve is reduced, lowering end-systolic pressure.
* Stroke Volume (SV) Modulation: While SV slightly increases due to reduced ESV, the primary benefit is decreased cardiac work rather than a large increase in CO. 

![](https://github.com/nickmmark/pressure-volume-loops/blob/main/HF_IABP.gif)

### Impella
Percutaneous Microaxial flow pumps (aka Impella devices) continuously pump blood from the left ventricle (LV) to the aorta, reduce LV preload (end-diastolic volume/pressure) and workload, resulting in a leftward shift and triangularization of the cardiac pressure-volume (PV) loop. This creates a smaller pressure-volume area (PVA), significantly lowering myocardial oxygen consumption. 
![](https://github.com/nickmmark/pressure-volume-loops/blob/main/impella.gif)

### VA-ECMO
Venoarterial extracorporeal membrane oxygenation (VA-ECMO) provides variable hemodynamic support depending on the pump RPM/flow rate. VA-ECMO causes a rightward and upward shift of the left ventricular (LV) pressure-volume (PV) loop, increasing end-diastolic volume/pressure and systolic pressure due to high afterload from retrograde arterial flow. Although VA-ECMO can provide more liters/minute of hemodynamic support, it has some detrimental effects:
* Upward and Rightward Shift: The PV loop moves to higher pressures and volumes, indicating increased LV end-diastolic pressure (LVEDP) and volume (LVEDV), which is indicative of LV distension.
* Increased Afterload: The arterial pressure generated by VA-ECMO acts as an increased afterload, which raises the effective arterial elastance (\(E_{a}\)) line on the PV diagram.
* Reduced Stroke Volume (SV): While total systemic flow increases, the LV's native SV decreases as it struggles to eject against high retrograde pressure.
* Increased Myocardial Oxygen Consumption (PVA): The Pressure-Volume Area (PVA) increases, reflecting higher energy expenditure by the heart despite the support.
* Decreased Loop Size: The overall PV loop area (representing cardiac work) may shrink, but this is often accompanied by dangerous overloading (distension) of the ventricle. 
...
* valvular pathologies (wide open AI, severe AS, etc)
